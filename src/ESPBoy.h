 ...
 * #if ESPBOY_LIB_VER >= 20100
 *   // ... code that make use of a new feature added to V2.1.0
 * #endif
 * \endcode
 */
#define ESPBOY_LIB_VER 50100

// EEPROM settings
#define ESPBOY_UNIT_NAME_LEN 6 /**< The maximum length of the unit name string. */
...
// monochrome OLED emulation on color LCD
// 0011 1000 0000 0111 -> 0111 0000 1000 0011
// RRRR RGGG GGGB BBBB -> RGGG RRRR BBBB GGGB
#define BACKGROUND_COLOR 0x0738 /**< Color value for an unlit pixel for draw functions. */
#define FOREGROUND_COLOR 0x9f07 /**< Color value for a lit pixel for draw functions. */
 ...
 *
 * \note
 * Only function ESPBoyBase::drawBitmap() currently supports this value.
 */
#define INVERT 2
 ...
 *
 * \see ESPBoyBase::collide(Point, Rect) ESPBoyBase::collide(Rect, Rect)
 */
struct Rect
{
 ...
 *
 * \see ESPBoyBase::collide(Point, Rect)
 */
struct Point
{
...
/** \brief
 * The main functions provided for writing sketches for the ESPBoy,
 * _minus_ text output.
 *
 * \details
 * This class in inherited by ESPBoy, so if text output functions are
 * required ESPBoy should be used instead.
 *
 * \note
 * \parblock
 * An ESPBoyAudio class object named `audio` will be created by the
 * ESPBoyBase class, so there is no need for a sketch itself to create an
 * ESPBoyAudio object. ESPBoyAudio functions can be called using the
 * ESPBoy or ESPBoyBase `audio` object.
 *
 * Example:
 *
 * \code{.cpp}
 ...
 * ESPBoy espboy;
 *
 * // ESPBoyAudio functions can be called as follows:
 *   espboy.audio.on();
 *   espboy.audio.off();
 * \endcode
 * \endparblock
 ...
 * \note
 * \parblock
 * A friend class named _ESPBoyEx_ is declared by this class. The intention
 * is to allow a sketch to create an _ESPBoyEx_ class which would have access
 * to the private and protected members of the ESPBoyBase class. It is hoped
 ...
 * \see ESPBoy
 */
class ESPBoyBase : public ESPBoyCore
{
 friend class ESPBoyEx;
 ...
 public:
   ...
   * ESPBoyAudio class object itself.
   *
   * \see ESPBoyAudio
   */
  ESPBoyAudio audio;
   ...
   * does not exit. The ESPBoy must be restarted after flashlight mode is used.
   *
   ...
   * after `boot()`.
   *
   * The ESPBoy logo scrolls down from the top of the screen to the center
   ...
   * it operates.
   *
   * \see begin() boot() bootLogoShell() ESPBoy::bootLogoText()
   */
  void bootLogo();
   ...
   * logo screen. The `ESPBoy` class uses this to display the unit name.
   *
   ...
   * to compensate.
   *
   * \see bootLogo() boot() ESPBoy::bootLogoExtra()
   */
   ...
   *
   * \code{.cpp}
   * if (espboy.everyXFrames(5)) {
   *   if espboy.pressed(A_BUTTON) {
   *     fireShot();
   *   }
   * }
   * \endcode
   ...
   *
   * \note
   * Sketches can use the defined value `ESPBOY_UNIT_NAME_LEN` instead of
   ...
   * char unitName[ESPBOY_UNIT_NAME_LEN + 1];
   *
   * // The actual name length
   * byte unitNameLength;
   *
   * unitNameLength = espboy.readUnitName(unitName);
   * \endcode
   *
   * \see writeUnitName() readUnitID() ESPBoy::bootLogoExtra()
   */
   ...
   *
   * \see readUnitName() writeUnitID() ESPBoy::bootLogoExtra()
   */
  ...
  void writeShowBootLogdisplaysFlag(bool val);
  bool readShowBootLogdisplaysFlag();
   ...
   * if (!moving) {
   *   if (espboy.justPressed(RIGHT_BUTTON)) {
   *     endMoving = espboy.frameCount + 10;
   *     moving = true;
   *   }
   * } else {
   *   movePlayer();
   *   if (espboy.frameCount == endMoving) {
   *     moving = false;
   *   }
   * }
   * \endcode
  ...
  static const uint32_t sBufferSize = (HEIGHT*WIDTH)/8;
  static uint8_t sBuffer[sBufferSize];
 ...
 *
 * \details
 * This class is derived from ESPBoyBase. It provides text output functions
 * in addition to all the functions inherited from ESPBoyBase.
 *
 * \note
 * A friend class named _ESPBoyEx_ is declared by this class. The intention
 * is to allow a sketch to create an _ESPBoyEx_ class which would have access
 * to the private and protected members of the ESPBoy class. It is hoped
 * that this may eliminate the need to create an entire local copy of the
 * library, in order to extend the functionality, in most circumstances.
 *
 * \see ESPBoyBase
 */
class ESPBoy : public Print, public ESPBoyBase
{
 friend class ESPBoyEx;
 ...
 public:
   ...
   *
   * \details
   * For an `ESPBoy` class object, functions provided by the Arduino `Print`
   ...
   * int value = 42;
   *
   * espboy.println("Hello World"); // Prints "Hello World" and then moves the
   *                                 // text cursor to the start of the next line
   * espboy.print(value);  // Prints "42"
   * espboy.print('\n');   // Moves the text cursor to the start of the next line
   * espboy.print(78, HEX) // Prints "4E" (78 in hexadecimal)
   * \endcode
   *
   * \see ESPBoy::write()
   */
   ...
   *
   * \details
   * This is the ESPBoy implemetation of the Arduino virtual `write()`
