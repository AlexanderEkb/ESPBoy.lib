...
#ifndef ESPBOY_CORE_H
#define ESPBOY_CORE_H
...
#if !defined(ESPBOY_10) && !defined(AB_DEVKIT)
/* defaults to ESPBoy Release 1.0 if not using a boards.txt file
 *
 * we default to ESPBoy Release 1.0 if a compile flag has not been
 * passed to us from a boards.txt file
 *
 * if you wish to compile for the devkit without using a boards.txt
 * file simply comment out the ESPBOY_10 define and uncomment
 * the AB_DEVKIT define like this:
 *
 *     // #define ESPBOY_10
 *     #define AB_DEVKIT
 */
#define ESPBOY_10   //< compile for the production ESPBoy v1.0
 ...
 * \endparblock
 *
 * \see ESPBoyCore::exitToBootloader()
 */
#define ESPBOY_NO_USB int main() __attribute__ ((OS_main)); \
int main() { \
  ESPBoyCore::mainNoUSB(); \
  return 0; \
}
 ...
 *
 * \details
 * This class is inherited by ESPBoyBase and thus also ESPBoy, so wouldn't
 ...
 * to the private and protected members of the ESPBoyCore class. It is hoped
...
class ESPBoyCore
{
  ...
  public:
	static const int8_t lcd_cs 	= 5;
	static const int8_t lcd_dc 	= 15;
     ...
     *_ESPBoyPlaytune_ is one such library known to do this.
     * The digitalWriteRGB() function will still work properly in this case.
     * \endparblock
     *
     * \note
     * \parblock
     * Many of the Kickstarter ESPBoys were accidentally shipped with the
    ...
    /** \brief
     * Initialize the ESPBoy's hardware.
     *
     * \details
     ...
     *
     * \see ESPBoyBase::flashlight() boot()
     */
