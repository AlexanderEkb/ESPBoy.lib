...
#ifndef ESPBOY_AUDIO_H
#define ESPBOY_AUDIO_H
 ...
 *
 * For example, the ESPBoyTones and ESPBoyPlaytune libraries require an
 * `enabled()` type function to be passed as a parameter in the constructor,
 * like so:
 *
 * \code{.cpp}
 * #include <ESPBoy.h>
 * #include <ESPBoyTones.h>
 *
 * ESPBoy espboy;
 * ESPBoyTones sound(espboy.audio.enabled);
 * \endcode
 * \endparblock
 *
 * \note
 ...
 * to the private and protected members of the ESPBoyAudio class. It is hoped
...
class ESPBoyAudio
{
 ...
 public:
   ...
   * system EEPROM. This function is called by `ESPBoyBase::begin()` so it
   * isn't normally required to call it within a sketch. However, if
   * `ESPBoyCore::boot()` is used instead of `ESPBoyBase::begin()` and the
